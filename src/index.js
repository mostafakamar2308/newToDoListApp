...
function removeSectionMenu(ele) {
  ...
  btn.addEventListener("click", async function () {
    ...
    await setDoc(doc(db, "Users", user), sectionObj);
...
function createProjectModal() {
  ...
  btn.addEventListener("click", async function h() {
...
async function markTaskInObject(ele, division) {
  let task =
    sectionObj[openedSection.split("-").join(" ")][division][
      openedProject.split("-").join(" ")
    ][ele];
...
let workk = 0;
let menu = 0;
async function markAllTasksInObj(hmmm) {
...
function completeMenu(ele) {
  ...
  markAsComplete.addEventListener("click", async function () {
